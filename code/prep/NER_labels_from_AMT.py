...
class AMT_labels():
	...
	def __init__(self):
		...
		self.fin = '../../data/AMT/labels/all_Reddit_AMT_labels.csv'
		self.fout = '../../data/AMT/labels/NER_Reddit_AMT_labels_href.csv'
		
		self.fout2 = '../../data/AMT/labels/filtered_Reddit_AMT_labels.csv'

	def __parse_posts(self, df, column_name):
	...
	def parse_all(self):
		...
		self.__parse_answers(self.df_in, 'Answer.symptoms')
		self.__parse_answers(self.df_in, 'Answer.drugs')
		self.__parse_posts(self.df_in, 'post')
		...
		with open(self.fout, 'w') as f:
			...
			for i, line in self.df_in.iterrows():
				syms = line['Answer.symptoms']
				drugs = line['Answer.drugs']
				text = line['post']
		...
		df_out.to_csv(self.fout2, columns=['Answer.symptoms', 'new_sym','Answer.drugs', 'new_drug', 'post'])
